$(function () {
    ...
    $.extend({
        TBbtnHtmlList: function (o1, o2, o3) {
            ...
            for (var i in o2) {
                tb_btn_html += "<button " + Identity_btn + " class=\"TBbtn " + o2[i].NewBtnUseCode + " " + o2[i].NewBtnClass + "\">" + o2[i].NewBtnName + " <i class=\"" + o2[i].NewBtnIco + "\"></i></button>";
            }
            return tb_btn_html;
        },
        TBbtnHtml: function (o1, o2, o3) {
            ...
            return "<button " + Identity_btn + " class=\"TBbtn " + o2[i].NewBtnUseCode + "" + o2.NewBtnClass + "\">" + o2.NewBtnName + " <i class=\"" + o2.NewBtnIco + "\"></i></button>";
        },
        ...
        $.fn.TBTgrid = function (options, object) {
            if (typeof (options) == "string") {
                ...
                switch (options) {
                    ...
                    case "getck":
                        ...
                        $("#tbody_" + This_ck_fatherId + "").find(".ckchild").each(function () {
                            ...
                            if (This_td_ck) {
                                var getval = ThisTR.parent("tr").find("span[field='" + object + "']").text();
                                ckArray.push(getval);
                            }
                ...
                var dft = {
                    //以下为该插件的属性及其默认值
                    rownumbers: true,
                    disabledShow: true,//当checkbox的属性为disabled时，是否显示复选框
                    rownotext: "",
                    TreeName: "",
                    ParentId: "",
                    Children: "Children",
                ...
                var Table_Html = " <!--startprint--> <table  style=\"table-layout: fixed;\" id=\"tb_Content_" + ThisId + "\"  class=\"ltable\"><tr id=\"head_" + ThisId + "\" class=\"tbheader\">";
                //$(ops.toolbar).remove();
                var Table_Html1 = "";
                if (ops.ck == true) {
                ...
                $("#tbody_" + ThisId).html("<tr><td style='padding:10px;' align=\"center\" colspan=\"" + Colspan + "\"><div style=\"width:130px;margin:0 auto;line-height: 40px;\"><img class=\"L\" width=\"40\" src=\"/Areas/BidMag/Content/v2images/loading.gif\">正在填充数据...</div></td></tr>");
                ...
                var MyFunction = {
                    ComputeChildrenCount: function (objCountChildren, outCount) {
                        if (objCountChildren.Children != null) {
                            outCount = objCountChildren.Children.length;
                            for (var CCount = 0; CCount < objCountChildren.Children.length; CCount++) {
                                if (objCountChildren.Children[CCount].Children != null) {
                                    outCount = outCount + objCountChildren.Children[CCount].Children.length;
                                }
                            }
                        }
                        return outCount;
                    },
                    ReadChildren: function (objChildren, thisIndex, Layer) {
                        ...
                        for (var ch = 0; ch < objChildren.length; ch++) {
                                ...
                                for (var ch_j = 0; ch_j < ops.columns[i].length; ch_j++) {
                                    ...
                                    if (ops.rownumbers) {
                                        if (ch_j == 0 && !ops.columns[i][ch_j].ignore && i == 0) {
                                            ...
                                            Tbody_Html_ch += "<td align=\"center\" width=\"40\">" + ch_index + "</td>";
                                        }
                                    }
                                    if (ops.columns[i][ch_j] != null) {
                                        ...
                                        if (ops.columns[i][ch_j].TBbtn != null) {
                                            if (typeof (ops.columns[i][ch_j].TBbtn) == "function") {
                                                ...
                                                tb_value = ops.columns[i][ch_j].TBbtn(objChildren[ch], "", ops.Btn_Identity);
                                            }
                                        }
                                        ...
                                        if (!ops.columns[i][ch_j].ignore) {
                                            ...
                                            if (ops.TreeName.toUpperCase() == UpperFild) {
                                                ...
                                                Tbody_Html_ch += "<td class='chrName PrName' " + width + " align=\"" + ops.columns[i][ch_j].align + "\"><span style=\"display:inline-block;width:" + (120 + (Layer * 20)) + "px;\"></span><span class='itemIco' style=\"display:inline-block;width:" + 18 + "px;\"></span><span class=\"folder-line\"></span><span class=\"folder-open\"></span><span class='' field=\"" + ops.columns[i][ch_j].field + "\">" + tb_value + "</span></td>";
                                            }
                                            else {
                                                if (ops.columns[i][ch_j].field == "ParentId") {
                                                ...
                                                else if (ops.columns[i][ch_j].field == "Id") {
                                                    Tbody_Html_ch += "<td " + width + " " + th_hidden + " style=\"white-space:nowrap;word-break:break-all;overflow:hidden;\" align=\"" + ops.columns[i][ch_j].align + "\"><span class='thisPid' field=\"" + ops.columns[i][ch_j].field + "\">" + tb_value + "</span></td>";
                                                }
                                                else {
                            ...
                            Tbody_Html_ch += "</tr>";
                            if (objChildren[ch].Children != null) {

                                Tbody_Html_ch += "<tr><td style=\"padding:0px;border: none;\" colspan=\"" + Colspan + "\"><table style=\"table-layout: fixed;clear: both;margin-top: 0 !important;margin-bottom: 0 !important; max-width: none !important;border:none;\" class=\"ltable\">" + MyFunction.ReadChildren(objChildren[ch].Children, ch_index, This_Children_Layer) + "</table></td></tr>";
                            }
                    ...
                    ConvertDataToChildren: function (fatherobj, childData) {
                        if (fatherobj != null) {
                            fatherobj.Children = new Array();

                            var c_i = 0;
                            for (var i = 0; i < childData.length; i++) {
                                if (fatherobj.Id == childData[i][ops.ParentId]) {

                                    fatherobj.Children[c_i] = childData[i];
                                    MyFunction.ConvertDataToChildren(fatherobj.Children[c_i], childData);
                                    c_i++;
                                }
                            }
                        }
                    ...
                    ReadHttpDataSetChildren: function (data) {
                        ...
                        for (var Http_i = 0; Http_i < data.rows.length; Http_i++) {
                            if (data.rows[Http_i][ops.ParentId] == null || data.rows[Http_i][ops.ParentId] == 0) {
                                ...
                                newArr.rows[Http_First_i].Children = new Array();
                                Http_First_i++;
                            }
                            else {
                            ...
                            if (data.total > 0) {
                                ...
                                for (var r = 0; r < data.rows.length; r++) {
                                    ...
                                    if (ops.ck == true) {
                                        ...
                                        if (data.rows[r].disabled && ops.disabledShow) {
                                        ...
                                        else if (data.rows[r].disabled && !ops.disabledShow) {
                                            Tbody_Html += "<td class=\"ckchild\" align=\"center\" width=\"40\"></td>"
                                        }
                                        else {
                                            Tbody_Html += "<td class=\"ckchild\" align=\"center\"><input " + ThisCK_disabled + " " + ThisCK_selected + " " + Identity_Html + " type=\"checkbox\" id=\"" + thisckId + "_input_ck\"/></td>"
                                        }
                                    ...
                                    if (data.rows[r][ops.Children] != null) {
                                        if (data.rows[r][ops.Children].length > 0) {
                                            Tbody_Html += "<tr><td colspan=\"" + Colspan + "\" style=\"padding:0px;border: none;\"><table style=\"table-layout: fixed;clear: both;margin-top: 0 !important;margin-bottom: 0 !important; max-width: none !important;;border:none;\" class=\"table table-striped table-bordered table-hover dataTable no-footer\">" + MyFunction.ReadChildren(data.rows[r][ops.Children], thisIndex, 1) + "</table></td></tr>";
                                        }
                                    }
                                    ...
                                    for (var i = 0; i < ops.columns.length; i++) {
                                        for (var j = 0; j < ops.columns[i].length; j++) {
                                            if (ops.columns[i][j] != null) {
                                                ...
                                                if (ops.columns[i][j].TBfmt != null) {
                                                    if (typeof (ops.columns[i][j].TBfmt) == "function") {
                                                        tb_value = ops.columns[i][j].TBfmt(tb_value);
                                                    }
                                                }
                                                else {
                        ...
                        $.AjaxSend("post", ops.url, nPara, function (data) {
                            ...
                            if (data.total > 0) {
                                ...
                                for (var r = 0; r < data.rows.length; r++) {
                                    ...
                                    Tbody_Html += "</tr>";
                                    if (data.rows[r][ops.Children] != null || data.rows[r].child != null) {
                                        Tbody_Html += "<tr><td colspan=\"" + Colspan + "\" style=\"padding:0px;border: none;\"><table style=\"table-layout: fixed;clear: both;margin-top: 0 !important;margin-bottom: 0 !important; max-width: none !important;;border:none;\" class=\"table table-striped table-bordered table-hover dataTable no-footer\">" + MyFunction.ReadChildren(data.rows[r][ops.Children], thisIndex, 1) + "</table></td></tr>";
                                    }
                                    $("#tbody_" + ThisId).append(Tbody_Html);
                                }
