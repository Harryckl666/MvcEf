...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	close: function()
	{
	    ...
	    that.closed = true;
        //2016-4-14加入此方法后，修复关闭窗口后，父页面出现滚动条
	    $(that.DOM.wrap).css("position", "fixed");
...
lhgdialog.prompt = function( content, yes, value, parent )
{
	...
	return lhgdialog({
		...
		content: [
			...
			'</div>',
			'<div>',
                  '<textarea name="textarea" style="width:300px;height:118px;">',value,'</textarea>',
			'</div>'
			].join(''),
		init: function(){
